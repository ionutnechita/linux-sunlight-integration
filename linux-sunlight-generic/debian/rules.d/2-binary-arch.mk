...
# Typically supplied from the arch makefile, e.g., debian.generic/control.d/armhf.mk
ifneq ($(gcc),)
...
$(stampdir)/stamp-prepare-tree-%: $(commonconfdir)/config.common.$(family) $(archconfdir)/config.common.$(arch) $(archconfdir)/config.flavour.% debian/scripts/fix-filenames
	...
	[ "$(do_full_source)" != 'true' ] && true || \
		rsync -a --exclude debian --exclude debian.generic --exclude $(DEBIAN) * $(builddir)/build-$*
...
binary-arch-headers: install-arch-headers
	...
	dh_testroot
ifeq ($(do_libc_dev_package),true)
ifeq ($(filter debian.generic%,$(DEBIAN)),)
...
ifeq ($(do_any_tools),true)
	...
	rsync -a --exclude debian --exclude debian.generic --exclude $(DEBIAN) --exclude .git -a ./ $(builddirpa)/
endif
	touch $@
